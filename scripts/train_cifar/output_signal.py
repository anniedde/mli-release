...
# vgg-19, no bn, SGD, lr 0.01 run 2
#run_num = 1577604 

run_num = 1577593 # vgg-19, no bn, SGD, lr 0.01 run 1
layers = [1,3,6,8,11,13,15,17,20,22,24,26,29,31,33,35]

path = "/usr/xtmp/CSPlus/VOLDNN/Annie/mli-release/output_signal_data/vgg19-noBN-sgd-lr0_01-alpha{}-expectation-run1/".format(str(alpha).replace(".", "_"))
print('path is ', path)
...
interpolated_output = {}
for layer in layers:
    interpolated_handles.append(model.features[layer].register_forward_hook(get_activation('features[{}]'.format(layer), interpolated_output)))
...
final_output = {}
for layer in layers:
    final_handles.append(model.features[layer].register_forward_hook(get_activation('features[{}]'.format(layer), final_output)))
